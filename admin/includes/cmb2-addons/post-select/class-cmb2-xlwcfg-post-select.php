 ...
 */
class CMB2_XLWCFG_Post_Select extends CMB2_Type_Multi_Base {

	public function render() {
		$a     = $this->parse_args( 'xlwcfg_post_select', array(
			'class'   => 'cmb2_select',
			'name'    => $this->_name(),
			'id'      => $this->_id(),
		...
		if ( $value && is_array( $value ) ) {
			foreach ( $value as $each ) {
				if ( ! array_key_exists( $each, $pre_options ) ) {
					$post_name        = ( isset( $field->args['options_name_cb'] ) ) ? call_user_func( $field->args['options_name_cb'], $each ) : $this->get_label_text($each);
	...
	public function get_label_text( $id ) {
