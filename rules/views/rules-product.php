...
$post_id = get_the_id();
$groups  = $this->get_entity_rules( $post_id, 'product' );
...
?>
<div class="xlwcfg-rules-builder woocommerce_options_panel" data-category="product">
    <div id="xlwcfg-rules-groups" class="xlwcfg_rules_common">
								...
								foreach ( $group as $rule_id => $rule ) :
                                    ...
                                    <tr data-ruleid="<?php echo $rule_id; ?>" class="xlwcfg-rule">
                                        <td class="rule-type"><?php
											...
											$args = array(
												'input'   => 'select',
												'name'    => 'xlwcfg_rule[product][' . $group_id . '][' . $rule_id . '][rule_type]',
												'class'   => 'rule_type',
												'choices' => $types,
											);
										...
										<?php
										XLWCFG_Core()->rules->ajax_render_rule_choice( array(
											'group_id'      => $group_id,
											'rule_id'       => $rule_id,
        ...
        <div class="xlwcfg_rules_bottom_note">
			...
			?>
            <a href="javascript:void(0)" onclick="show_modal_pro('product_rules_modal');" target="_blank"><?php _e( 'PRO version', 'bonanza-woocommerce-free-gifts-lite' ); ?>.</a>
        </div>
    </div>
</div>
<script type="text/template" id="xlwcfg-rule-template-product">
